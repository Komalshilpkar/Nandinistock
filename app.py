...
with st.sidebar:
    ...
    lookback_override = st.slider("Override lookback (0 = auto)", min_value=0, max_value=120, value=0)
    training_epochs = st.slider("Training epochs (sequence models)", min_value=1, max_value=20, value=3)
    show_company_profile = st.checkbox("Show Company Profile", value=True)
    ...
    if auto_weight:
        ...
        weight_window = 0
    manual_weights_enabled = st.checkbox("Manually set ensemble weights", value=False)
    manual_weights = {}
    if manual_weights_enabled:
        st.markdown("**Manual ensemble weights** — set relative weight per model")
        # Show simple number inputs for common candidate models
        w_lstm = st.number_input("LSTM weight", min_value=0.0, max_value=10.0, value=1.0)
        w_gru = st.number_input("GRU weight", min_value=0.0, max_value=10.0, value=1.0)
        w_prophet = st.number_input("Prophet weight", min_value=0.0, max_value=10.0, value=1.0)
        w_sma = st.number_input("SMA weight", min_value=0.0, max_value=10.0, value=1.0)
        manual_weights = {
            'LSTM (1-layer)': float(w_lstm),
            'GRU (1-layer)': float(w_gru),
            'Prophet': float(w_prophet),
            'Simple Moving Average': float(w_sma)
        }
    st.markdown('---')
    # Settings persistence
    settings_path = Path('.dashboard_settings.json')
    if st.button('Save Settings'):
        try:
            s = {
                'lookback_override': int(lookback_override),
                'training_epochs': int(training_epochs),
                'show_company_profile': bool(show_company_profile),
                'show_confidence': bool(show_confidence),
                'manual_weights_enabled': bool(manual_weights_enabled),
                'manual_weights': manual_weights
            }
            settings_path.write_text(json.dumps(s))
            st.success('Settings saved to .dashboard_settings.json')
    ...
    if settings_path.exists():
        if st.button('Load Settings'):
            try:
                ...
                st.info('Settings loaded — please re-run analysis to apply them')
            except Exception as e:
...
MODEL_ERROR_HISTORY_PATH = Path('.model_error_history.json')


def load_error_history():
    """Load historical model errors by ticker and model name."""
    try:
        if MODEL_ERROR_HISTORY_PATH.exists():
            hist = json.loads(MODEL_ERROR_HISTORY_PATH.read_text())
...
def save_error_history(history):
    """Persist model error history to disk (JSON)."""
    try:
        MODEL_ERROR_HISTORY_PATH.write_text(json.dumps(history, indent=2))
    except Exception:
        pass


def get_model_error(symbol, model_name, error_history):
    """Retrieve cached error for a model on a given ticker; fallback to average if not found."""
    if symbol not in error_history:
        return None
    if model_name not in error_history[symbol]:
        return None
    errs = error_history[symbol][model_name]
    if not isinstance(errs, list) or len(errs) == 0:
        return None
    # Use most recent error (last in list)
    return float(errs[-1])


def record_model_error(symbol, model_name, error_value, error_history, max_history=10):
    """Add new error to history and limit to last max_history entries."""
    if symbol not in error_history:
        error_history[symbol] = {}
    if model_name not in error_history[symbol]:
        error_history[symbol][model_name] = []
    error_history[symbol][model_name].append(float(error_value))
    # Keep only last max_history entries
    if len(error_history[symbol][model_name]) > max_history:
        error_history[symbol][model_name] = error_history[symbol][model_name][-max_history:]
...
if st.button("🚀 Run Full Analysis"):
    try:
        ...
        if show_company_profile:
            ...
            company_summary = info.get('longBusinessSummary') or info.get('businessSummary') or "No company summary available."
            logo_url = info.get('logo_url') or info.get('logo') or None
            sector = info.get('sector')
            industry = info.get('industry')
            employees = info.get('fullTimeEmployees')
            website = info.get('website')
            country = info.get('country') or info.get('state')
            market_cap = info.get('marketCap')

            st.subheader("🏢 Company Profile")
            cp_col1, cp_col2 = st.columns([1, 3])
            # Logo and key facts
            try:
                if logo_url:
                    cp_col1.image(logo_url, width=120)
            except Exception:
                pass

            cp_col1.markdown(f"**{company_name}**")
            if sector:
                cp_col1.markdown(f"- **Sector:** {sector}")
            if industry:
                cp_col1.markdown(f"- **Industry:** {industry}")
            if employees:
                try:
                    cp_col1.markdown(f"- **Employees:** {int(employees):,}")
                except Exception:
                    cp_col1.markdown(f"- **Employees:** {employees}")
            if market_cap:
                try:
                    cp_col1.markdown(f"- **Market Cap:** {int(market_cap):,}")
                except Exception:
                    cp_col1.markdown(f"- **Market Cap:** {market_cap}")
            if website:
                try:
                    cp_col1.markdown(f"- [Website]({website})")
                except Exception:
                    cp_col1.markdown(f"- Website: {website}")

            # Long description
            try:
                cp_col2.markdown(company_summary)
            except Exception:
                cp_col2.markdown("No detailed company description available.")
        ...
        with st.spinner("🤖 Training & Predicting..."):
            # Load error history for intelligent weighting
            ...
            if use_ensemble:
                ...
                model_errors = {}
                # If auto-weight requested, compute errors (may be slow); otherwise try cached history
                if auto_weight and weight_window > 0:
                    with st.spinner("🧪 Running short backtest to estimate model accuracy..."):
                        for mname in candidate_models:
                            err = compute_backtest_error(mname, eval_window=weight_window)
                            model_errors[mname] = err
                            # Update persistent history
                            record_model_error(symbol, mname, err, error_history)
                else:
                    # Try to use cached errors from previous runs (faster)
                    for mname in candidate_models:
                        cached_err = get_model_error(symbol, mname, error_history)
                        if cached_err is not None:
                            model_errors[mname] = cached_err
                ...
                if len(preds_list) > 0:
                    # If auto_weight or cached errors: compute normalized inverse-error weights
                    if (auto_weight or len(model_errors) > 0) and len(model_errors) > 0:
                        weights = []
        ...
        st.divider()
        vspace(1)
        # Top: Latest data + Prediction side-by-side
        with st.container():
            ...
            with right:
                st.subheader("🤖 10-Day Prediction")
        ...
        with st.container():
            ...
            with c1:
                st.metric("Current Price", f"₹{round(last_price,2)}")
                st.metric("Shares (est)", f"{int(shares)}")
            with c2:
                st.write(f"**Target Price:** ₹{round(target_price,2)}  ")
                st.write(f"**Stop Loss:** ₹{round(stop_loss,2)}  ")
                st.write(f"**Predicted (10d):** ₹{round(future_prices[-1][0],2)}  ")
                st.write(f"**Expected P/L:** ₹{round(expected_profit,2)}")
        vspace(1)
